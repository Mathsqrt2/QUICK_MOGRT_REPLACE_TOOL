class Property{
    ...
    drawPropertyValue = (origin,property) =>{
        if(typeof(property.value) == "string"){
            ...
            const range = this.nItem(sliderBox,["input","range",`${property.value}`],"rangeSelector");
                ...
                range.addEventListener("change",(e)=>this.processChanges(property.name,e.target.value));
            return numberBox;
        }
    } 
    ...
    drawPropertyHeader = (origin) =>{
        ...
        const showProperties = this.nItem(header,["input","button","unwrap"]);
              showProperties.classList.add("showPropertiesButton");
                        this.nItem(header,["p",`${this.name} (${JSON.parse(this.instances).length})`],"propertyHeaderDescription");
    ...
    processChanges = (property,newValue) => {
        cs.evalScript(`$.mogrts_control.processReplacement('${this.name}','${property}','${newValue}')`);
    }
